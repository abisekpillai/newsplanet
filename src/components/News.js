...
export class News extends Component {
  ...
  async componentDidMount(){
    let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=8362708e62074e7cac98b7b12db02254&page=1&pageSize=${this.props.pageSize}`;
  ...
  handleNextClick = async () =>{
    ...
    if(!(this.state.page+1>Math.ceil(this.state.totalResults/12))){
      let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=8362708e62074e7cac98b7b12db02254&page=${this.state.page+1}&pageSize=${this.props.pageSize}`;
      this.setState({ loading : true});
      let data = await fetch(url);
      let parsedData = await data.json();
      this.setState({ articles : parsedData.articles,
